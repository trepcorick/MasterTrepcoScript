#!/bin/bash
# Trepco Masternode Setup Script V1.4 for Ubuntu 16.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'trepcod' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop Trepcod${NC}"
        trepco-cli stop
        sleep 30
        if pgrep -x 'trepco' > /dev/null; then
            echo -e "${RED}Trepcod daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            pkill trepcod
            sleep 30
            if pgrep -x 'trepcod' > /dev/null; then
                echo -e "${RED}Can't stop trepcod! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${YELLOW}Trepco Masternode Setup Script V1.5 for Ubuntu 16.04 LTS${NC}"
...
wget https://github.com/trepcorick/trepco/releases/download/v1.1.1/trepco-1.1.1-ubuntu-16.04-compiled.tar.gz
...
rm -rf trepco-1.1.1-ubuntu-16.04-compiled.tar.gz
 ...
 sudo cp ~/MasterTrepcoScript/trepco/trepco* /usr/bin/
 sudo chmod 755 -R ~/MasterTrepcoScript
 sudo chmod 755 /usr/bin/trepco*

 # Deploy masternode monitoring script
 cp ~/MasterTrepcoScript/trepcomon.sh /usr/local/bin
 sudo chmod 711 /usr/local/bin/trepcomon.sh

 #Create datadir
 if [ ! -f ~/.trepco/trepco.conf ]; then
 	sudo mkdir ~/.trepco
 fi
clear
echo -e "${YELLOW}Creating trepco.conf...${NC}"
...
echo -ne '\n'
    ...
    genkey=$(trepco-cli masternode genkey)
    if [ -z "$genkey" ]; then
    ...
    #Stopping daemon to create trepco.conf
    stop_daemon
    sleep .5
fi
# Create trepco.conf
...
#Finally, starting daemon with new trepco.conf
trepcod --daemon
sleep 5

#Setting auto start cron job for trepcod
cronjob="@reboot sleep 30 && trepcod"
crontab -l > tempcron
...
To introduce your new masternode to the Trepco network, you need to
...
Currently your masternode is syncing with the Trepco network...
...
========================================================================
To view masternode configuration produced by this script in trepco.conf:
${YELLOW}cat ~/.trepco/trepco.conf${NC}
Here is your trepco.conf generated by this script:
-------------------------------------------------${YELLOW}"
cat ~/.trepco/trepco.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit trepco.conf, first stop the trepcod daemon,
then edit the trepco.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the trepcod daemon back up:
             to stop:   ${YELLOW}trepco-cli stop${NC}
             to edit:   ${YELLOW}nano ~/.trepco/trepco.conf${NC}
             to start:  ${YELLOW}trepcod${NC}
========================================================================
To view trepco debug log showing all MN network activity in realtime:
             ${YELLOW}tail -f ~/.trepco/debug.log${NC}
========================================================================
...
sync status etc. in real-time, run the nodemon.sh script:
                 ${YELLOW}trepcomon.sh${NC}
or just type 'node' and hit <TAB> to auto-complete script name.
========================================================================
Enjoy your Trepco Masternode and thanks for using this setup script!
"
sleep 30
# Run trepcomon.sh
trepcomon.sh
